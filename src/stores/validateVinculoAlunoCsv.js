export const requiredColumnsVinculoAluno = [
  'Matrícula*',
  'Nome do Aluno*',
  'Disciplina (Código)*',
  'Turma*'
...
export const validateVinculoAlunoCsv = (csvData) => {
  ...
  const missingColumns = requiredColumnsVinculoAluno.filter(col => !csvColumns.includes(col));
  ...
  const validRows = csvData.data.filter((row, index) => {
    ...
    requiredColumnsVinculoAluno.forEach(column => {
